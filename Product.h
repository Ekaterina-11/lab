#include <string>

using namespace std;
class Product
{
private:
    string name;
    short size;
    string color;
public:
    Product();
    Product(string name, short size, string color);
    Product(const Product &product);
    void printInformationAboutProduct() const ;
    void setName(string name_in);
    void setSize(short size_in);
    void setColor(string color_in);
    string getName() const;
    short getSize() const;
    string getColor() const;
    Product& operator ++ (); //префиксный инкремент
    Product& operator -- (); // префиксный декремент
    Product operator ++ (int); //постфиксный инкремент
    Product operator -- (int); // постфиксный декремент
    int operator == (const Product product11) const;//создание оператора равенства
    int operator != (const Product product11) const;//создание оператора неравенства
    int operator > (const Product product11) const;//создание оператора больше
    int operator < (const Product product11) const;//создание оператора меньше
    int operator >= (const Product product11) const;//создание оператора больше или равно
    int operator <= (const Product product11) const;//создание оператора меньше или равно
    Product& operator = (const Product product) const;//создание оператора присваивания

    friend ostream& operator << (ostream &out, const Product &product_in) const;//создание оператора вывода
    friend istream& operator >> (istream &in, Product &product_in);//создание оператора ввода
};
